...
Citizen.CreateThread(function()
    while true do
        ...
        if drawCurrentHand then 
            castle = 4
            --drawNativeNotification("Sua mão é ["..tostring(currentHand).."], a mão do dealer é ["..tostring(dealersHand).."]")
            drawTxt("Sua mão é ~g~["..tostring(currentHand).."]~w~, a mão do dealer é ~b~["..tostring(dealersHand).."]~w~",4,0.5,0.93,0.50,255,255,255,180)
            --AddTextComponentString("Sua mão é ["..tostring(currentHand).."], a mão do dealer é ["..tostring(dealersHand).."]")
        end
        Citizen.Wait(castle)
    end
end)
...
Citizen.CreateThread(function()
    while true do 
        ...
        for k,v in pairs(cfg.blackjackTables) do
            ...
            if cfg.blackjackTables[k].distance < 10.0 then
                closeToCasino = true
            end
        end
        Wait(1000)
...
RageUI.CreateWhile(1.0, true, function()
    if RageUI.Visible(RMenu:Get('cmgblackjack', 'instructions')) then
        RageUI.DrawContent({ header = true, glare = true, instructionalButton = true }, function()           
            RageUI.FakeButton("test", "O objetivo do Blackjack é vencer a mão do dealer sem ultrapassar 21. Este jogo usa quatro baralhos de 52 cartas, que são embaralhados no início de cada mão. O dealer ficará no soft de 17. Aperte [E] para entrar na mesa.", { RightLabel = "→→→" }, true, function(Hovered, Active, Selected)
                if (Hovered) then
...
Citizen.CreateThread(function()
    while true do
        ...
        if blackjackInstructional ~= nil then 
            ...
            if waitingForBetState then 
                DrawRect(0.933, 0.828, 0.125, 0.032, 0, 0, 0, 150) 
                DrawAdvancedNativeText(0.991, 0.835, 0.005, 0.0028, 0.29, "APOSTA:", 255, 255, 255, 255, 0, 0)
                DrawAdvancedNativeText(1.041, 0.828, 0.005, 0.0028, 0.464, tostring(currentBetAmount), 255, 255, 255, 255, 0, 0)
                --DrawTimerBar2("BET",tostring(currentBetAmount),1)
            end
        end
        if drawTimerBar then
            castle = 4
            DrawRect(0.944, 0.786, 0.101, 0.032, 0, 0, 0, 150)
            DrawAdvancedNativeText(1.013, 0.792, 0.005, 0.0028, 0.29, "TEMPO:", 255, 255, 255, 255, 0, 0)
            if timeLeft < 10 then
                --DrawTimerBar2("TIME","00:0" .. tostring(timeLeft),1)
                DrawAdvancedNativeText(1.05, 0.785, 0.005, 0.0028, 0.464, "00:0" .. tostring(timeLeft), 255, 255, 255, 255, 0, 0)
            else
                --DrawTimerBar2("TIME","00:" .. tostring(timeLeft),1)
                DrawAdvancedNativeText(1.05, 0.785, 0.005, 0.0028, 0.464, "00:" .. tostring(timeLeft), 255, 255, 255, 255, 0, 0)
            end
        end
...
function setupBlackjackInstructionalScaleform(scaleform)
    ...
    ButtonMessage("Diminuir aposta") --Page Down
    PopScaleformMovieFunctionVoid()
