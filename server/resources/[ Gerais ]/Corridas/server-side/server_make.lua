...
-----------------------------------------------------------------------------------------------------------------------------------------
mkVRP = {}
Tunnel.bindInterface("Make",mkVRP)
vmCLIENT = Tunnel.getInterface("Make")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local races = {}
...
RegisterCommand("races",function(source,args,rawCommand)
	...
	if user_id then
		if parseInt(args[1]) > 0 and races[parseInt(args[1])] then
			vmCLIENT.selectRaces(source,parseInt(args[1]))
			playerRaces[source] = parseInt(args[1])
		elseif args[1] == "make" then
			vmCLIENT.makeRaces(source)
		end
	end
end)
-----------------------------------------------------------------------------------------------------------------------------------------
-- INPUTRACE
-----------------------------------------------------------------------------------------------------------------------------------------
function mkVRP.inputRaces(status)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
...
-- INPUTRACE
-----------------------------------------------------------------------------------------------------------------------------------------
function mkVRP.finishRaces(raceSelected)
	local source = source
	local user_id = vRP.getUserId(source)
	if user_id then
		...
		for k,v in pairs(playerRaces) do
			if parseInt(raceSelected) == parseInt(v) then
				async(function()
					vmCLIENT.finishRaces(k,parseInt(raceSelected))
