	...
	if user_id then
		for k,v in pairs(Farms.itemList) do
			...
			if vRP.getInventoryItemAmount(user_id,v.item) >= parseInt(rand) then
				...
				if (v.item == "weed" or v.item == "lean" or v.item == "meth" or v.item == "ecstasy" or v.item == "cocaine" or v.item == "lsd") and math.random(100) >= 70 then
...
function cnVRP.paymentMethod()
	local source = source
	local user_id = vRP.getUserId(source)
	local bonus = vRP.bonusDelivery(user_id)
	if user_id then
		if vRP.tryGetInventoryItem(user_id,amount[user_id][1],amount[user_id][2],true) then
			...
			if parseInt(bonus) >= 1 then
				local qntd = parseInt(value) * parseInt(bonus) / 10
				vRP.giveInventoryItem(user_id,"dollars2",parseInt(qntd),true)
				TriggerClientEvent("Notify",source,"sucesso","Gorjeta de $"..qntd.." recebida.",4000)
			end
...
function cnVRP.sendNotify(x,y,z)
	local source = source
