...
vtdSERVER = Tunnel.getInterface("towdriver")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local blip = nil
...
Citizen.CreateThread(function()
	while true do
		...
		if not IsPedInAnyVehicle(ped) then
			...
			local init = configs.towdriver.init
			local distance = #(coordsPed - vector3(init[1],init[2],init[3]))
			if distance <= 10 then
				timeDistance = 4
				DrawMarker(21,init[1],init[2],init[3]-0.6,0,0,0,0.0,0,0,0.5,0.5,0.4,240,203,88,50,0,0,0,1)
				if IsControlJustPressed(1,38) and distance <= 2 and not inService then
					startthreadservicetow()
...
-- THREADSERVICE
-----------------------------------------------------------------------------------------------------------------------------------------
function startthreadservicetow()
	Citizen.CreateThread(function()
		while true do
			local timeDistance = 500
			if inService then
				...
				local deliveryVeh = configs.towdriver.deliveryVeh
				local distance = #(coords - vector3(deliveryVeh[1],deliveryVeh[2],deliveryVeh[3]))
				if distance <= 30 then
					timeDistance = 4
					DrawMarker(23,deliveryVeh[1],deliveryVeh[2],deliveryVeh[3]-0.95,0,0,0,0,0,0,14.0,14.0,1.0,240,203,88,50,0,0,0,0)
					if IsControlJustPressed(1,38) and not IsPedInAnyVehicle(ped) and timeSeconds <= 0 and distance <= 10 and towed == nil then
						...
						if IsEntityAVehicle(vehicle) and vehName then
							if vehName:lower() == vehicles[vehSelect][1] then
								vehicleNet = 0
								inService = false
								vtdSERVER.paymentMethodTow()
	...
	while true do
		...
		if inService then
			...
			if IsControlJustPressed(1,168) then
				...
				inService = false
				TriggerServerEvent("DeleteVehicle",VehToNet(vehicle))
				TriggerEvent("Notify","negado","Emprego de <b>Guincho</b> finalizado.",5000)
...
function twVRP.towPlayer()
	...
	if IsVehicleModel(vehicle,GetHashKey("flatbed")) and not IsPedInAnyVehicle(PlayerPedId()) then
		...
		if DoesEntityExist(vehicle) and DoesEntityExist(towed) then
			if tow then
				vtdSERVER.tryTow(VehToNet(vehicle),VehToNet(tow),"out")
				towed = nil
				tow = nil
			else
				if vehicle ~= towed then

					vtdSERVER.tryTow(VehToNet(vehicle),VehToNet(towed),"in")
					tow = towed
				end
			end
		end
...
-- CHECKSERVICE
-----------------------------------------------------------------------------------------------------------------------------------------
function twVRP.checkServiceTow()
	return inService
end