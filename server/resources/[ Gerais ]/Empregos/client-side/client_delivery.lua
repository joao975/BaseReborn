...
vSERVERdel = Tunnel.getInterface("delivery")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
local locates = {}
...
Citizen.CreateThread(function()
	while true do
		...
		if not IsPedInAnyVehicle(ped) then
			...
			local init = configs.delivery.init
			local distance = #(coords - vector3(init[1],init[2],init[3]))
			if distance <= 2.5 then
				timeDistance = 4
				DrawText3DHeight(init[1],init[2],init[3],"~g~E~w~   EMPACOTAR   [ ~y~"..string.upper(locates).."~w~ ]",450)
				if distance <= 0.6 and IsControlJustPressed(1,38) then
					if vSERVERdel.dropFood(1,locates) then
						inService = true
						inLocate = 1
						inSelected = math.random(#delivery[inLocate])
						makeBlipMarkedDelivery()
					else
						locates = itemList[math.random(#itemList)]
					end
				end
	...
	while true do
		...
		if not IsPedInAnyVehicle(ped) then
			...
			if inService then
				...
				if distance <= 30 then
					timeDistance = 4
					DrawText3DHeight(delivery[inLocate][inSelected][1],delivery[inLocate][inSelected][2],delivery[inLocate][inSelected][3],"~g~E~w~   ENTREGAR PACOTE",400)
					if distance <= 0.6 and IsControlJustPressed(1,38) then
						vSERVERdel.paymentMethodDelivery()
						if DoesBlipExist(serviceBlip) then
							RemoveBlip(serviceBlip)
							serviceBlip = nil
							Citizen.Wait(1000)
...
-- DrawText3DHeight
-----------------------------------------------------------------------------------------------------------------------------------------
function DrawText3DHeight(x,y,z,text,height)
...
-- makeBlipMarkedDelivery
-----------------------------------------------------------------------------------------------------------------------------------------
function makeBlipMarkedDelivery()
	if DoesBlipExist(serviceBlip) then
		RemoveBlip(serviceBlip)
		serviceBlip = nil
	end
