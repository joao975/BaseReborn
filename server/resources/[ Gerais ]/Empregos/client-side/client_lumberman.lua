...
vlbSERVER = Tunnel.getInterface("lumberman")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIABLES
-----------------------------------------------------------------------------------------------------------------------------------------
...
Citizen.CreateThread(function()
	while true do
		...
		if not IsPedInAnyVehicle(ped) then
			...
			local init = configs.lumberman.init
			local distance = #(coords - vector3(init[1],init[2],init[3]))
			if distance <= 2.5 then
				timeDistance = 4
				DrawText3D(init[1],init[2],init[3],"~g~E~w~ LENHADOR",450)
				if distance <= 1.0 and IsControlJustPressed(1,38) then
					toggleServiceLenhador()
					Citizen.Wait(5000)
				end
			end
		end
...
function toggleServiceLenhador()
	if inService then
		inService = false
		...
		startthreadservicelumber()
		startthreadservicesecondslm()
		inService = true
	...
	while true do
		...
		if inService then
			...
			if IsControlJustPressed(1,168) then
				...
				TriggerEvent("Notify","negado","Emprego de <b>Lenhador</b> finalizado.",5000)
			end
		end
		Citizen.Wait(will)
	end
...
-- THREADSERVICE
-----------------------------------------------------------------------------------------------------------------------------------------
function startthreadservicelumber()
	Citizen.CreateThread(function()
		while true do
			local timeDistance = 500
			if inService then
				...
				if not IsPedInAnyVehicle(ped) then
					...
					if collectDis <= 30 then
						...
						if collectDis <= 0.6 and IsControlJustPressed(1,38) and timeSeconds <= 0 then
							if GetSelectedPedWeapon(ped) == GetHashKey("WEAPON_HATCHET") then
								timeSeconds = 2
								if vlbSERVER.collectMethodLumber() then
								...
								TriggerEvent("Notify","aviso","Você precisa de um machado",5000)
							end
						end
					end
					...
					if deliverDis <= 100 then
						...
						if deliverDis <= 0.6 and IsControlJustPressed(1,38) and timeSeconds <= 0 then
							timeSeconds = 2
							if vlbSERVER.paymentMethodLenhador() then
								deSelected = math.random(#deliver)
								deliverBlipMarked()
							end
						end
...
-- TIMESECONDS
-----------------------------------------------------------------------------------------------------------------------------------------
function startthreadservicesecondslm()
	Citizen.CreateThread(function()
		while true do
