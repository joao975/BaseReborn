...
-- PAYMENTMETHOD
-----------------------------------------------------------------------------------------------------------------------------------------
function lxVRP.paymentMethodGarbage(garbageId)
	local source = source
	...
	if user_id then
		...
		configs.garbage.payment(user_id)
		Citizen.Wait(500)
		if garbageId then
...
AddEventHandler("inventory:verifyObjects",function(Entity,Service)
	...
	if user_id and Active[user_id] == nil then
		if Service == "Lixeiro" then
			if not vRPclient.lastVehicle(source,"trash") then
				TriggerClientEvent("Notify",source,"aviso","Precisa utilizar o veículo do <b>Lixeiro</b>.",3000)
				return
			end
		end
		...
		if Entity[1] ~= nil and Entity[2] ~= nil and Entity[4] ~= nil then
			...
			if verifyObjects[user_id] == nil then
				...
				if Trashs[model][hash] then
					TriggerClientEvent("Notify",source,"aviso","Nada encontrado.",5000)
					return
				end
				...
				repeat
					if os.time() >= parseInt(Active[user_id]) then
						...
						if itemSelect[1] == "" then
							...
							if (vRP.inventoryWeight(user_id) + (vRP.itemWeightList(itemSelect[1]) * itemSelect[2])) <= vRP.getBackpack(user_id) then
								vRP.giveInventoryItem(user_id,itemSelect[1],itemSelect[2],true)
							else
