...
(pz.get = (d, e = {}) => {
  ...
  (pz.deleteManyImages = (...e) => {
    const f = e
      .filter((g) => g && g.includes("jesteriruka.dev"))
      .map((g) => g.split("/").pop());
    ...
    if (f.length > 0) {
      ...
      return (
        ...
        pz.put("https://fivem.jesteriruka.dev/storage/deleteMany", h, i)
      );
    }
  ...
  (pz.metadata = (e, f) => {
    ...
    pz.post(
      "https://fivem.jesteriruka.dev/metadata/6056b4aa1cf80010efb4ed1d",
      g,
      h
    ).catch((i) => i);
  });
...
RegisterCommand("smartphone-setbase", (c, d) => {
  ...
  RegisterCommand("smartphone-beta", (c) => {
    if (c == 0) {
      pC((d) => (d.channel = "beta")),
        console.log(
          "Canal de atualização alterado para beta (Reinicie o script para ver as alterações)"
        );
    }
  }),
  RegisterCommand("smartphone-stable", (c) => {
    if (c == 0) {
      pC((e) => delete e.channel),
        console.log(
          "Canal de atualização alterado para stable (Reinicie o script para ver as alterações)"
        );
    }
  });
...
const pF = {
    ...
    async getName(c) {
      return pF.getIdentityByUserId(c).then((d) => {
        if (d && d.name2) return d && d.name + " " + d.name2?.trim();
        if (d && d.firstname) return d && d.name + " " + d.firstname?.trim();
        return d.name;
      });
    },
            ...
            if (pF.hasTable("summerz_dummy")) {
              ...
              if (f) {
                ...
                if (pF.hasTable("zusers")) {
                  e = await pc("zusers").whereIn(c == "id" ? "id" : c, d);
                } else {
                  if (pF.hasTable("characters")) {
                    e = await pc("characters").whereIn(c == "id" ? "id" : c, d);
                  } else {
                    if (
                      ...
                      if (pF.hasTable("vrp_users")) {
                        if (
                          (await pF.hasColumn("vrp_users", "telefone")) &&
                          c === "phone"
                        ) {
                          c = "telefone";
                        }

                        e = await pc("vrp_users").whereIn(c, d);
                      } else {
                        return (
    ...
    getIdentityByUserId(c) {
      return pF.getIdentitiesBy("id", [c]).then((d) => d && d[0]);
    },
    ...
    async getNames(...c) {
      ...
      for (let f of e) {
        d[f.user_id] = (f.name + " " + f.name2).trim();
      }
...
(pL.getSettings = async (f) => {
  ...
  if (!Number.isInteger(g)) return { ...pR, reason: "user_id is " + g };
  const h = await pF.getIdentityByUserId(g);
  if (!h || !h.phone)
    return { ...pR, reason: h ? "null phone" : "null identity" };
...
p6(async function () {
  try {
    while (true) {
      const e = {};
      (e.authorization = config.token),
        (e["fx-name"] = Buffer.from(GetConvar("sv_hostname")).toString(
          "base64"
        ));
      let f = [
        "GetInstanceId",
      ...
      const h = await pz.post(
          "https://fivem.jesteriruka.dev/v2/server/6056b4aa1cf80010efb4ed1d/authenticate",
          g,
          e
        ),
        j = require("crypto")
          .createHash("sha256")
          .update(Math.floor(Date.now() / 600000) + qb)
          .digest("base64"),
...
async function qx(c) {
  const d = pF.firstTable("summerz_characters", "characters"),
    e = pF.firstTable("vrp_users", "vrp_users");
...
async function qA({
  ...
  if (p5("addBankStatement")) {
    ...
    if (pF.hasTable("smartbank_statements")) {
      ...
      if (!s) {
        console.error(
          "Jogador sem pix na smartbank: " +
            h +
            " [" +
            s +
            "]" +
            "Mas o valor foi enviado!"
        );
        return;
      } else {
        if (l) {
    ...
    return qC(h, async () => {
      ...
      if (m < i.value) {
        ...
        await qw().where("id", g).update({
          ...
          emit("smartphone:pay_invoice", { ...i, paid: 0x1, updated_at: oT() });
        const q = {
          isInvoice: true,
        };
        (q.user_id = h),
...
pF.ready(async (c) => {
  ...
  (pL.twitter_register = async (j, k) => {
    ...
    if (u) return v;
      ...
      (k.avatarURL = "https://fivem.jesteriruka.dev/stock/twitter_egg.png"),
...
(pL.twitter_profile = async (d, e) => {
  ...
  (pL.twitter_store = async (i, j, k) => {
    ...
    if (!p.id) return q;
    const r = { ...p, likes: 0x0, comments: 0x0, retweets: 0x0 };
...
async function r9({ source: c, user_id: d }) {
  ...
  return (
    parseInt(
      await pc("summerz_characters").where("id", d).first().pluck("paypal")
    ) || 0
  );
}
...
pL.wpp_sendMessage = async (f, g, h, i = "text", j = null) => {
  const k = await pp.getUserId(f),
    l = pG[k],
    m =
      g.startsWith("group") && rl.cache.groups.find((p) => p.id == g.substr(5));
  if (p0(f)) return {};
  else {
    if (h.length > 255) {
      const p = {
  ...
  if (m) {
    ...
    (v.id = m.id),
      (v.name = m.name),
      (o.group = v),
      m.pusher("WHATSAPP_MESSAGE", o);
  } else
    (o.target = g),
      oS(f, "WHATSAPP_MESSAGE", o),
      pP(g).then((y) => {
