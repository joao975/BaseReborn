...
RegisterNetEvent('Vape:UseGusto')

AddEventHandler("Vape:UseGusto",function(item, usage) 
	GustoLoaded = {
		item = item,
		usage = usage
	}
	...
	if DoesEntityExist(ped) and not IsEntityDead(ped) then
		if IsPedOnFoot(ped) then
			if IsPlayerAbleToVape == false then
				...
				ShowNotification("Você começou a usar seu vape.")
				Wait(Config.HelpTextLength)
				TriggerEvent("Vape:HelpFadeOut", 0)
			else
				ShowNotification("Você já está segurando seu vape.")
			end
		else
			ShowNotification("Você não pode fumar no veículo.")
		end
	else
		ShowNotification("Você não pode fumar se estiver morto.")
	end
end)
...
AddEventHandler("Vape:StopVaping", function(source)
	if IsPlayerAbleToVape == true then
		...
		ShowNotification("Você parou de usar seu vape.")
	else
		ShowNotification("Você não está segurando seu vape.")
	end
end)
AddEventHandler("Vape:Drag", function()
	if IsPlayerAbleToVape then
		if GustoLoaded ~= false then
			if GustoLoaded.usage > 0 then
				GustoLoaded.usage = GustoLoaded.usage - 1
				...
				GustoLoaded = false
				ShowNotification("Você está sem líquido.")
			end
		else
			ShowNotification("Você não tem líquido carregado.")
		end
	else
		ShowNotification("Você tem que segurar seu vape na mão.")
	end
end)
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPlayerAbleToVape then
			...
			if IsControlPressed(0, Config.StopVape) then
	...
	while true do
		if IsPlayerAbleToVape then
			if DisplayText then
				local ped = GetPlayerPed(-1)
				local pedPos = GetEntityCoords(ped)
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.20, "~c~Segure ~b~\"E\"~c~ para fumar.")
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+1.08, "~c~Segure ~b~\"G\"~c~ para abaixar o vape.")
				DrawText3d(pedPos.x, pedPos.y, pedPos.z+0.96, "~c~Pressionne ~b~\"X\"~c~ para parar de fumar.")
			end
		end
	  Wait(1)
	end
