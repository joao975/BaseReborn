  ...
  function changeImage() {
    ...
    checkImageExistence(imagePath, function (exists) {
      ...
      if (exists) {
        ...
        newImage.attr(
          "src",
          folderPath + filePrefix + currentIndex + fileextension
        );
        $("#video-foreground").append(newImage);
        newImage.hide().fadeIn(1000);
      }
...
$(document).ready(function () {
  ...
  $("#staff-title").html(
    Locales[Config.Locale]["stafftitle"] +
      "&nbsp" +
      '<span id="staff-quantity" class="staff-quantity">' +
      count +
      "</span>"
  );
            ...
            <div class="staff-image">${firstInitial}${secondInitial}</div>
              <div class="staff-info-div">
                  ...
                  <div class="staff-rank">${
                    Locales[Config.Locale][staffMember.rank]
                  }</div>
              </div>
          </div>
          `;
  ...
  $(".patch-notes-banner").css(
    "background-image",
    "url(img/patchnotes/" + Config.UpdateList[lastUpdate].image + ")"
  );
  ...
  $("#updates-banner").attr(
    "src",
    "img/patchnotes/" + Config.UpdateList[lastUpdate].image
  );
      ...
      if (Config.UseYoutubeVideo) {
        if (isAlternate) {
          buttonContainer.innerHTML =
            '<i class="fa-solid fa-video button-i2"></i>';
          player.playVideo();
        } else {
          buttonContainer.innerHTML =
            '<i class="fa-solid fa-video-slash button-i2"></i>';
          player.pauseVideo();
        }
...
function toggleMusicWidget(event) {
  ...
  if (!musicWidgetOpened) {
    ...
    if (isMusicWidgetVisible) {
      ...
      `;
      $(".music-current-time, .music-total-time").css(
        "animation",
        "moveUp 0.65s ease-in-out"
      );
      $(".music-name, .music-artist, .music-progress-bar, .music-controls").css(
        "animation",
        "moveLeft 0.65s ease-in-out"
      );
...
$(document).on("click", function (event) {
  ...
  if (
    event.target !== musicManagement &&
    !musicManagement.contains(event.target)
  ) {
    if (isMusicWidgetVisible) {
      ...
      $(".music-current-time, .music-total-time").css(
        "animation",
        "moveDown 0.25s ease-in-out"
      );
      $(".music-name, .music-artist, .music-progress-bar, .music-controls").css(
        "animation",
        "moveRight 0.25s ease-in-out"
      );
      ...
      setTimeout(() => {
        $(
          "#music-name, #music-artist, #music-current-time, #music-total-time, #music-progress-bar, #music-controls"
        ).html("");
      }, 250);
      setTimeout(() => {
  ...
  if (
    event.target !== openStaffButton &&
    !openStaffButton.contains(event.target) &&
    event.target !== staffMenu &&
    !staffMenu.contains(event.target) &&
    staffMenuOpened
  ) {
    staffMenuOpened = false;
    toggleStaffMenu();
  ...
  if (
    event.target !== openUpdatesButton &&
    !openUpdatesButton.contains(event.target) &&
    event.target !== updatesMenu &&
    !updatesMenu.contains(event.target) &&
    updatesMenuOpened
  ) {
    updatesMenuOpened = false;
    toggleUpdatesMenu();
...
function previousUpdate() {
  ...
  setTimeout(() => {
    ...
    $("#patch-notes-banner-title").html(Config.UpdateList[lastUpdate].title);
    $("#patch-notes-title").html(Config.UpdateList[lastUpdate].subtitle);
    $("#patch-notes-description").html(
      Config.UpdateList[lastUpdate].description
    );
    $("#updates-banner").attr(
      "src",
      "img/patchnotes/" + Config.UpdateList[lastUpdate].image
