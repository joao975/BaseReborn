...
#################################################################
                ...
                AJUDA NA CONFIG DE SCRIPTS ESX/QBCORE

#################################################################

Primeiramente confere se esta sendo importado os modulos dessa forma:
----------------------------------------------------------------------------------

    QBCore = exports["qb-core"]:GetCoreObject()
    ESX = exports["es_extended"]:getSharedObject()

Se estiver, troque por esse:
----------------------------------------------------------------------------------
...
Outro detalhe é o banco de dados. Troque as tabelas respectivas:

`users`     ->  `vrp_users`     [ identifier = id ]

`players`   ->  `vrp_users`     [ citizenid = id ]

`bans`      ->  `user_bans`     [ citizenid = id ]

--- EXEMPLO ---

MySQL.query('UPDATE players SET xp = xp + @xp WHERE identifier = @identifier', {
...
MySQL.Async.fetchAll('UPDATE vrp_users SET xp = xp + @xp WHERE id = @identifier', {
			['@identifier'] = xPlayer.PlayerData.citizenid,
			['@xp'] = xp,
		}, function (result)
		end)
