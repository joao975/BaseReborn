...
async(function()
	local cloakroom_types = Reborn.cloakroom_types()
	for k,v in pairs(cloakroom_types) do
		local menu = { name = k }
		menus[k] = menu
...
local function build_client_points(source)
	local cloakrooms = Reborn.cloakrooms()
	for k,v in pairs(cloakrooms) do
		local gtype,x,y,z = table.unpack(v)
		local cloakroom_types = Reborn.cloakroom_types()
		local cloakroom = cloakroom_types[gtype]
		...
		if cloakroom and menu then
			...
			local function cloakroom_enter(source,area)
				...
				if user_id and vRP.hasAnyPermission(user_id,gcfg.permissions or {}) then
