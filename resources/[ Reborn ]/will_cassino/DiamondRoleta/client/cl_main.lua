...
createRulettAsztal = function(index, data)
    ...
    --(string.format('Rulett table creating.. %s', self.index))
                                    ...
                                    if IsDisabledControlJustPressed(0, 24) then
                                        if currentBetAmount > 0 then
                                            if mesasRoletas[selectedRulett] ~= nil then
                                                if currentBetAmount >= mesasRoletas[selectedRulett].minBet and currentBetAmount <= mesasRoletas[selectedRulett].maxBet then
                                                    ...
                                                    TriggerEvent("Notify","negado","Sua aposta é muito pequena, ou muito grande para essa mesa.",5000)
                                                    TriggerEvent("Notify","aviso","Minimo: "..mesasRoletas[selectedRulett].minBet.." Maximo: "..mesasRoletas[selectedRulett].maxBet.."",5000)
                                                end
                                            end
                                        else
                                            TriggerEvent("Notify","negado","Sua aposta é invalida.",5000)
                                        end
                                    end
                                end
                            end
...
Citizen.CreateThread(
    function()
        while true do
            ...
            closetoRulett = false
            for k, v in pairs(mesasRoletas) do
                if #(playerCoords - mesasRoletas[k].position) < 12.0 then
                    closetoRulett = true
                end
            end
            Wait(1000)
                ...
                for k, v in pairs(Rulettek) do
                    if DoesEntityExist(v.tableObject) then
                        ...
                        if dist < 2.5 then
                            ...
                            DrawText3Ds(closestChairData.position.x, closestChairData.position.y, closestChairData.position.z + 1.3,'~g~E~w~ para se sentar.')
...
AddEventHandler(
    ...
    function(rulettIndex, tickRate)
        if Rulettek[rulettIndex] ~= nil then
            --(string.format('rulett table index: %s, tickrate: %s', rulettIndex, tickRate))
