...
Citizen.CreateThread(function()
	while true do
		...
		closetoSlots = false
		for k, v in pairs(SlotsMachines) do
			if #(playerCoords - SlotsMachines[k].pos) < 15.0 then
				closetoSlots = true
			end
		end
		Citizen.Wait(1000)
		...
		if state then
			...
			Citizen.CreateThread(function()
				while selectedSlot ~= nil do
					...
					drawTxt('VALOR PARA GIRAR: ~g~' ..self.data.bet..' '..itemBNT:upper(), 4,0.10,0.850,0.50,255,255,255,180)
					drawTxt('~g~E~w~ PARA TROCAR A VISÃO', 4,0.10,0.90,0.50,255,255,255,180)
					drawTxt('~g~SPACE~w~ PARA APOSTAR', 4,0.10,0.925,0.50,255,255,255,180)
					drawTxt('~r~BACKSPACE~w~ PARA SAIR.', 4,0.10,0.95,0.50,255,255,255,180)
				end
			end)
            Citizen.Wait(2000)
		else
			...
			for k, v in pairs(Slots) do
				if DoesEntityExist(v.tableObject) then
					...
					if dist < 2.0 then
						...
						DrawText3Dss(closestChairData.position.x, closestChairData.position.y, closestChairData.position.z + 1.3,'~g~E~w~ para se sentar.')
