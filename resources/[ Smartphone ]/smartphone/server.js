...
const pF = {
  ...
  async 'getName'(c) {
    return pF.getIdentityByUserId(c).then(d => d && d.name + ' ' + d.name2.trim());
  },
  ...
  async 'getIdentitiesBy'(c = 'id', d = []) {
    ...
    let e;
    const f = pF.firstTable('vrp_users', 'vrp_characters', 'vrp_characterdata');
    if (pF.hasTable('nyo_character')) e = await pc('nyo_character').whereIn(c === 'id' ? 'id' : c, d).select('id', 'name', 'last_name', 'phone');else {
      if (pF.hasTable('drip_characters')) return c = c == 'id' ? 'id' : 'phone_number', e = await pc('drip_characters').whereIn(c, d).select('id', 'name', 'phone_number'), e.map(k => {
        const [l, m] = k.name.split(' ', 2),
              n = String(k.phone_number || k.id + 1000),
              o = {};
        return o.user_id = k.id, o.name = l, o.name2 = m || '', o.phone = n, o;
      });else {
        if (pF.hasTable('summerz_characters')) c = c == 'id' ? 'id' : c, e = await pc('summerz_characters').whereIn(c, d).select('id', 'name', 'name2', 'phone');else {
          if (pF.hasTable('summerz_dummy')) {
            c = c == 'id' ? 'id' : c, e = await pc('characters').whereIn(c, d).select('id', 'name', 'name2', 'phone');
          } else {
            if (f) {
              ...
              if (pF.hasTable('zusers')) {
                ...
                if (pF.hasTable('characters')) {
                  ...
                  if ((await pF.hasColumn('vrp_users', 'phone')) && (await pF.firstColumn('vrp_users', 'name', 'nome', 'firstname', 'name2'))) {
                    ...
                    } else e = await pc('vrp_users').whereIn(c == 'id' ? 'id' : c, d);
                  } else {
                    if (pF.hasTable('vrp_users')) {
                      if ((await pF.hasColumn('vrp_users', 'telefone')) && c === 'phone') {
                        c = 'telefone';
                      }
...
async function qx(c) {
  ...
  const f = await pc(d || e).where(d ? 'id' : 'id', c).first('chavePix');
  return f?.['chavePix'];
}
...
async function qy(d, e) {
  ...
  const f = await pM(d),
        g = ['vrp_user_moneys', 'summerz_bank', 'vrp_users', 'vrp_users_infos', 'vrp_characters', 'vrp_characterdata', 'vrp_user_identities', 'vrp_users', 'zusers', 'drip_characters', 'summerz_characters', 'characters', 'nyo_character'];
...
pF.ready(async () => {
  ...
  for (let [k, l, m] of [['vrp_fines', 'text', 'price'], ['characters_fines', 'text', 'price'], ['vRP:multas', 'motivo', 'valor'], ['rb_multas', 'motivo', 'valor'], ['vRP:multas', 'motivo', 'valor']]) {
    e.push({
      'table': k,
