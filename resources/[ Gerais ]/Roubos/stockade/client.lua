...
vsSERVER = Tunnel.getInterface("Rstockade")
-----------------------------------------------------------------------------------------------------------------------------------------
-- VARIÁVEIS
-----------------------------------------------------------------------------------------------------------------------------------------
		...
		if not IsPedInAnyVehicle(ped) then
			...
			if DoesEntityExist(vehicle) and GetEntityModel(vehicle) == GetHashKey("stockade") then
				...
				if distance <= 2.5 and blockStockades[plate] == nil then
					...
					if IsControlJustPressed(1,38) and distance <= 1.0 and vsSERVER.checkPolice(plate) then
						SetEntityHeading(ped,GetEntityHeading(vehicle))
						vsSERVER.withdrawMoney(plate,VehToNet(vehicle))
					end
				end
			end
		end
...
-- FREEZEPLAYERS
-----------------------------------------------------------------------------------------------------------------------------------------
function cjVRP.freezePlayers(status)
